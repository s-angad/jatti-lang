...
function activate(context) {
    ...
    outputChannel.appendLine('Jatti Language Extension v0.1.0 initialized');
    
    // Show info message
    vscode.window.showInformationMessage('🚀 Jatti Language extension loaded!');
    ...
    let runCommand = vscode.commands.registerCommand('jatti.run', () => {
        ...
        const cliPath = path.join(fileDir, 'cli.py');
        
        if (!fs.existsSync(cliPath)) {
            vscode.window.showErrorMessage(
                '❌ cli.py not found. Make sure you have the full Jatti language package. ' +
                'Visit: https://github.com/s-angad/jatti-lang'
            );
            outputChannel.appendLine('❌ Error: cli.py not found at ' + cliPath);
            return;
        }
...
function runJattiFile(filePath) {
    ...
    const process = execFile('python', [cliPath, 'run', filePath], {
        cwd: fileDir,
        maxBuffer: 10 * 1024 * 1024
    }, (error, stdout, stderr) => {
        if (stdout && typeof stdout === 'string') outputChannel.append(stdout);
        if (stderr && typeof stderr === 'string') outputChannel.append(stderr);
        outputChannel.append('\n==================================================\n');
...
function runInTerminal(filePath) {
    ...
    terminal.sendText(`python cli.py run ${fileName}`);
}
...
function buildJattiFile(filePath) {
    ...
    execFile('python', [cliPath, 'build', filePath], {
        cwd: fileDir,
        maxBuffer: 10 * 1024 * 1024
    }, (error, stdout, stderr) => {
        if (stdout && typeof stdout === 'string') outputChannel.append(stdout);
        ...
        if (error && error.code !== 0) {
            ...
            const pythonFile = filePath && typeof filePath === 'string' ? filePath.replace(/\.jatti$/, '.py') : 'output.py';
...
function formatJattiFile(filePath) {
    ...
    execFile('python', [cliPath, 'format', filePath], {
        cwd: fileDir,
        maxBuffer: 10 * 1024 * 1024
    }, (error, stdout, stderr) => {
        if (stdout && typeof stdout === 'string') outputChannel.append(stdout);
